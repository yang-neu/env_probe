...
def main():
    ...
    try:
        ...
        sql = 'select * from {} order by probe_date desc;'.format(internalEnvTable)
        print(sql)
        ...
        # id, probe_date, temp, humi
            for row in rows:
                sql = "insert into {} values ({},'{}',{},{});".format(internalEnvTable, row[0], row[1], row[2], row[3])
                print(sql)
                curs.execute(sql)
            conn_o.commit()
            curs.close()
